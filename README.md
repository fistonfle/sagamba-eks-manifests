...
| `postgres.yaml` | PostgreSQL + PVC (storageClass: gp3) |
...
- **Storage**: PVCs use `storageClassName: gp3`; change to `gp2` or your default if needed
...
PVCs use `storageClassName: gp3`. If your EKS default is different or you use a custom driver, set `storageClassName` in each PVC or set a default StorageClass in the cluster.
